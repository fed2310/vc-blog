...
exports.createPages = async ({ graphql, actions }) => {
    ...
    const podcastsingle = result.data.allGhostPost.edges
    const index = [];
    ...
    const podcastSingleTemplate = path.resolve(`./src/templates/podcastsingle.js`)
    const amppostTemplate = path.resolve(`./src/templates/post.amp.js`)
    // const amppodcastTemplate = path.resolve(`./src/templates/post.amp.js`)
    ...
    posts.forEach(({ node }) => {
        ...
        if(node.primary_tag){
            // console.log(node.primary_tag)
           var primary_tag = node.primary_tag.name
        }else{
            var primary_tag = ""
        }
        // console.log(node.tags)
        if(node.tags && node.tags.length >0 && node.tags[0].name !== '#Podcasts'){
    ...
    podcastsingle.forEach(({ node }) => {
        node.url = `/podcasts/${node.slug}/`
        // console.log(node.tags)
        if(node.tags && node.tags.length >0 && node.tags[0].name === '#Podcasts'){                
            createPage({
                path: node.url,
                component: podcastSingleTemplate,
                context: {
                    slug: node.slug,
                },
            })
    ...
    podcastsingle.forEach(({ node }) => {
        ...
        node.url = `/podcasts/${node.slug}/amp/`

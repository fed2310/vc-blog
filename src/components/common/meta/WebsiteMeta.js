...
const WebsiteMeta = ({ data, settings, canonical, title, description, image, type, urldata }) => {
    ...
    if (urldata) {
        title = `${title || data.meta_title || data.name || data.title} - ${settings.title}`+` | Page-`+urldata
        description = `${description || data.meta_description || data.description || config.siteDescriptionMeta || settings.description}`+` | Page-`+urldata
    } else {

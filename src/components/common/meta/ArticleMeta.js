...
const ArticleMetaGhost = ({ data, settings, canonical, isamp }) => {
    ...
    // console.log(isamp);
    let published_date = new Date(ghostPost.published_at)
    const published_at = published_date.toISOString()

    let updated_date = new Date(ghostPost.updated_at)
    const updated_at = updated_date.toISOString()
    ...
    return (
        <>
            <Helmet>
                ...
                {!isamp && <link rel="canonical" href={canonical} />}

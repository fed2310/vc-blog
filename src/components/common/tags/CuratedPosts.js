...
const CuratedPosts = ({src}) => {

    const data = useStaticQuery(graphql`
    query MyQuery3 {
        allGhostPost(sort: {order: DESC, fields: published_at}, limit: 6, filter: {tags: {elemMatch: {name: {eq: "#curated-for-you"}}}}) {
            nodes {
                id
                title
                slug
    ...
    return (
        <div className="row">
            {items}
            <div className="row"><a className="tags-explorebtn mrauto btn_get btn_hover" href="/tag/curated-for-you/">Explore more</a></div>
        </div>
    )
}
